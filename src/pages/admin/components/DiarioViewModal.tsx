...
export function DiarioViewModal({ 
  ...
  return (
    <>
      {/* 
          CORREÇÃO DO OVERLAY:
          Mantive o seu código original e apenas ajustei o overlay para 'fixed inset-0' e 'z-[60]'.
          Isso garante que o fundo preto ocupe 100% da tela, ignorando margens de elementos pai.
      */}
                  ...
                  {aulas.map((aula) => (
                    ...
                    >
                      <div className="flex items-center gap-2 text-blue-600 mb-2">
                        ...
                        <span className="text-sm font-bold">{new Date(aula.data).toLocaleDateString('pt-BR')}</span>
                      </div>
                      ...
                      <tr className="bg-gray-50">
                        ...
                        {avaliacoes.map(av => (
                          <th key={av.id} className="border p-3 text-xs font-bold text-gray-600 uppercase text-center min-w-[80px]">
                            {av.titulo}
                            <br />
                            <span className="text-[10px] font-normal lowercase">Peso: {av.peso}</span>
                          </th>
                        ))}
                    ...
                    <tbody>
                      {alunos.map((aluno) => (
                        <tr key={aluno.numero} className="hover:bg-gray-50">
                          ...
                          <td className="border p-3 text-sm text-center font-bold text-gray-900">
                            {aluno.media?.toFixed(1) || '-'}
                          </td>
                          <td className="border p-3 text-sm text-center text-gray-500">
                            {aluno.faltas || '-'}
                          </td>
      ...
      {/* Modal de Devolver Diário - INLINE */}
      {isDevolverOpen && (
