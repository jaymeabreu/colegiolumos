...
export function DiarioViewModal({ 
  ...
  return (
    <>
      {/* 
          CORREÇÃO DO OVERLAY:
          Removi a div extra que estava envolvendo o modal e causando margens.
          O overlay agora usa 'fixed inset-0' e 'z-[60]' para garantir que cubra 100% da tela,
          ignorando qualquer padding ou margin de elementos pai.
      */}
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] backdrop-blur-sm">
        <div 
          className="bg-background rounded-xl !w-[95vw] !max-w-none flex flex-col shadow-2xl overflow-hidden border h-[95vh]"
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-blue-50 to-indigo-50 flex-shrink-0">
            ...
            <button 
              ...
              type="button"
              className="p-2 hover:bg-white rounded-full transition-colors text-gray-600 hover:text-gray-900 cursor-pointer"
            >
              <X className="h-6 w-6" />
            </button>
          </div>
          ...
          {/* CONTEÚDO COM SCROLL */}
          <ScrollArea className="flex-1 overflow-auto bg-white">
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="bg-gray-50">
                        ...
                        <th className="border p-3 text-xs font-bold text-gray-600 uppercase text-left">Nome</th>
                        {avaliacoes.map(av => (
                    ...
                    <tbody>
                      {alunos.map((aluno) => (
                        <tr key={aluno.numero} className="hover:bg-gray-50">
                          ...
                          <td className="border p-3 text-sm font-medium">{aluno.nome}</td>
                          {avaliacoes.map(av => (
                          ...
                          <td className="border p-3 text-sm text-center font-bold">{aluno.media?.toFixed(1) || '-'}</td>
                          <td className="border p-3 text-sm text-center text-gray-500">{aluno.faltas || '-'}</td>
                          <td className="border p-3 text-center">
                            <Badge className={`text-[10px] uppercase font-bold ${aluno.media && aluno.media >= 7 ? 'bg-green-100 text-green-700 border-green-200' : 'bg-yellow-100 text-yellow-700 border-yellow-200'}`}>
          ...
          <div className="p-6 border-t bg-gray-50 flex-shrink-0">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <Button variant="outline" onClick={() => onOpenChange(false)} className="bg-white cursor-pointer">Fechar</Button>
                {onExportar && <Button variant="outline" onClick={onExportar} className="bg-white cursor-pointer"><Download className="h-4 w-4 mr-2" />Exportar PDF</Button>}
                {canDevolver && (
                  <Button variant="outline" className="border-orange-200 text-orange-700 hover:bg-orange-50 cursor-pointer" onClick={() => setIsDevolverOpen(true)}>
                    <RotateCcw className="h-4 w-4 mr-2" />Devolver
                  </Button>
                )}
                {canFinalizar && onFinalizar && (
                  <Button className="bg-green-600 hover:bg-green-700 cursor-pointer" onClick={onFinalizar} disabled={loading}>
                    <CheckCircle className="h-4 w-4 mr-2" />Finalizar
                  </Button>
                )}
                {canDesfinalizar && (
                  <Button variant="outline" className="border-red-200 text-red-700 hover:bg-red-50 cursor-pointer" onClick={handleDesfinalizar} disabled={desfinalizado || loading}>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[70] backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-md mx-4 overflow-hidden border border-gray-200">
            <div className="p-6 border-b flex items-start justify-between">
              <div><h2 className="text-xl font-bold text-gray-900 mb-1">Devolver Diário</h2><p className="text-sm text-gray-600">Deseja devolver este diário?</p></div>
              <button onClick={() => setIsDevolverOpen(false)} className="p-1 hover:bg-gray-100 rounded-full text-gray-400"><X className="h-5 w-5" /></button>
            </div>
            <div className="p-6">
              <textarea value={motivoDevolucao} onChange={(e) => setMotivoDevolucao(e.target.value.slice(0, 500))} placeholder="Motivo..." className="w-full px-3 py-2 border rounded-lg h-32 bg-white" />
            </div>
            <div className="border-t bg-gray-50 px-6 py-4 flex gap-3 justify-end">
              <Button variant="outline" onClick={() => setIsDevolverOpen(false)} className="bg-white">Cancelar</Button>
              <Button className="bg-[#1e4e5f] hover:bg-[#153a47] text-white" onClick={handleDevolverDiario} disabled={devolvendoDiario}>{devolvendoDiario ? 'Devolvendo...' : 'Devolver'}</Button>
            </div>
          </div>
        </div>
      )}
